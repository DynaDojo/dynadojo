"""
Attraction-Repulsion Weighted Hegselmann-Krause (ARWHK)
"""
...
class ARWHKSystem(OpinionSystem):
    ...
    Example
    ---------
    >>> from dynadojo.systems.opinion import ARWHKSystem
    ...
    >>> timesteps = 40
    >>> n = 1
    >>> system = SystemChecker(ARWHKSystem(dim, embed_dim, epsilon=0.3, noise_scale=0.0005))
    >>> x0 = system.make_init_conds(n=n)
    >>> x = system.make_data(x0, timesteps=timesteps)
    >>> plot([x], target_dim=2, max_lines=100)

    .. image:: ../_images/arwhk.png
    ...
    def __init__(self, latent_dim=2, embed_dim=2,
        ...
        """
        Initializes a ARWHKSystem instance.
        ...
        p_stubborn : float
            Percentage of stubborn agents
        option_stubborn : int
            Define distribution of stubborns: -1: negative, 0: positive, 1: neutral
        p_edge : float
            Probability for every pair of agents that an edge will be created between them
        method : int
            The variant of System to apply: 1-> With attraction 2-> With repulsion, 3-> With attractio and repulsion
        noise_scale : float
