...
class SISSystem(EpidemicSystem):
    ...
    Example
    ---------
    >>> from dynadojo.systems.epidemic import SISSystem
    ...
    >>> timesteps = 20
    >>> n = 1
    >>> system = SystemChecker(SISSystem(dim, embed_dim,  p_removal=0.1, noise_scale=0.1))
    >>> x0 = system.make_init_conds(n=n)
    >>> x = system.make_data(x0, timesteps=timesteps)
    >>> plot([x], target_dim=2, max_lines=100)

    .. image:: ../_images/sis.png
    ...
    def __init__(self, latent_dim=2, embed_dim=2,
        ...
        """
        Initializes a SISSystem instance.
        ...
        embed_dim : int
            If group_status is False, must be the same as latent_dim; if group_status is True, must be equal to number of statuses for the system (SIS: 2)
        IND_range : tuple
            In-distribution range of initial possible values, {0: Susceptible, 1: Infected}.
            We add +1 to the max range value to use np.floor to ensure all values are equally sampled from.
        OOD_range : tuple
            Out-of-distribution range of initial possible values, {0: Susceptible, 1: Infected}.
            We add +1 to the max range value to use np.floor to ensure all values are equally sampled from.
        p_infection : float
