    ...
    def make_data(self, init_conds: np.ndarray, timesteps: int, pts_per_period=500, control=None, noisy=False, return_times=False):
        ...
        if return_times: 
            ...
            for i in range(n):
                ...
                timepoint, trajectory = self.system.make_trajectory(timesteps, resample=True, pts_per_period = pts_per_period, return_times=True)
                if trajectory.shape[0] < timesteps:
                    timepoint, trajectory = self.system.make_trajectory(timesteps, resample=False, pts_per_period = pts_per_period, return_times=True)
        ...
        for i in range(n):
            self.system.ic = init_conds[i]
            trajectory = self.system.make_trajectory(timesteps, resample=True, pts_per_period = pts_per_period)
            if trajectory.shape[0] < timesteps:
                trajectory = self.system.make_trajectory(timesteps, resample=False, pts_per_period = pts_per_period)
            assert trajectory.shape == (timesteps, self._embed_dim)
            trajectories[i] = trajectory
