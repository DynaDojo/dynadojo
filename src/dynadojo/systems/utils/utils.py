...
class FBSNNSystem(AbstractSystem):
    ...
    def __init__(self, latent_dim, embed_dim,
        ...
        self.tf.compat.v1.disable_v2_behavior()
    ...
    def _initialize_NN(self, layers):
        ...
        for l in range(0,num_layers-1):
            ...
            b = self.tf.Variable(self.tf.zeros([1,layers[l+1]], dtype=self.tf.float32), dtype=self.tf.float32)
            weights.append(W)
            biases.append(b)
    ...
    def _xavier_init(self, size):
        ...
        return self.tf.Variable(self.tf.random.truncated_normal([in_dim, out_dim],
                                               stddev=xavier_stddev), dtype=self.tf.float32)
    ...
    def _neural_net(self, X, weights, biases):
        ...
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
            H = self.tf.sin(self.tf.add(self.tf.matmul(H, W), b))
        W = weights[-1]
        b = biases[-1]
        Y = self.tf.add(self.tf.matmul(H, W), b)
        return Y
    
    def _net_u(self, t, X): # N x 1, N x latent_dim
        u = self._neural_net(self.tf.concat([t,X], 1), self.weights, self.biases) # N x 1
        Du = self.tf.gradients(u, X)[0] # N x latent_dim
        
        return u, Du

    def _Dg_tf(self, X): # N x latent_dim
        return self.tf.gradients(self._g_tf(X), X)[0] # N x latent_dim
    ...
    def _loss_function(self, t, W, X0, noisy): # N x (timesteps) x 1, N x (timesteps) x latent_dim, 1 x latent_dim
        ...
        for n in range(0,self.timesteps):
            t1 = t[:,n+1,:]
            W1 = W[:,n+1,:]
            X1 = X0 + self._mu_tf(t0,X0,Y0,Z0)*(t1-t0) + self.tf.squeeze(self.tf.matmul(self._sigma_tf(t0,X0,Y0),self.tf.expand_dims(W1-W0,-1)), axis=[-1])
            Y1_tilde = Y0 + self._phi_tf(t0,X0,Y0,Z0)*(t1-t0) + self.tf.reduce_sum(Z0*self.tf.squeeze(self.tf.matmul(self._sigma_tf(t0,X0,Y0),self.tf.expand_dims(W1-W0,-1))), axis=1, keepdims = True)
            Y1, Z1 = self._net_u(t1,X1)
            
            loss += self.tf.reduce_sum(self.tf.square(Y1 - Y1_tilde))
        ...
        loss += self.tf.reduce_sum(self.tf.square(Y1 - self._g_tf(X1)))
        loss += self.tf.reduce_sum(self.tf.square(Z1 - self._Dg_tf(X1)))

        X = self.tf.stack(X_list,axis=1)
        Y = self.tf.stack(Y_list,axis=1)
    ...
    def make_data(self, init_conds: np.ndarray, control: np.ndarray, timesteps: int, noisy=False) -> np.ndarray:
        ...
        # tf session
        self.sess = self.tf.compat.v1.Session(config=self.tf.compat.v1.ConfigProto(allow_soft_placement=True,
                                                     log_device_placement=False))

        # tf placeholders and graph (training)
        self.learning_rate = self.tf.compat.v1.placeholder(self.tf.float32, shape=[])
        self.t_tf = self.tf.compat.v1.placeholder(self.tf.float32, shape=[self.N, self.timesteps+1, 1]) # N x (timesteps) x 1
        self.W_tf = self.tf.compat.v1.placeholder(self.tf.float32, shape=[self.N, self.timesteps+1, self.latent_dim]) # N x (timesteps) x latent_dim
        self.Xi_tf = self.tf.compat.v1.placeholder(self.tf.float32, shape=[self.N, self.latent_dim]) # 1 x latent_dim

        self.loss, self.X_pred, self.Y_pred, self.Y0_pred = self._loss_function(self.t_tf, self.W_tf, self.Xi_tf, noisy)
    
        # initialize session and variables
        init = self.tf.compat.v1.global_variables_initializer()
        self.sess.run(init)
    ...
    def _sigma_tf(self, t, X, Y): # N x 1, N x latent_dim, N x 1
        return self.tf.linalg.diag(self.tf.ones([self.N,self.latent_dim])) # N x latent_dim x latent_dim