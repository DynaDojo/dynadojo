...
class SNNChallenge(LDSChallenge):
    ...
    def _make_data(self, init_conds: np.ndarray, 
        ...
        def get_trajectory(x0, u):
          u_interp = interp1d(time, u, kind=3) # TODO: only works if u[-1] = u(t_f)
        ...
        data = Parallel(n_jobs=4)(delayed(get_trajectory)(x0, u) for x0,u in init_conds)
...
class SpikingNeuralNetwork:
  ...
  def simulate(self, x_0, t_eval):
    ...
    rv_0 = np.hstack([np.linalg.pinv(self.Phi)@x_0, np.zeros(self.N)])
