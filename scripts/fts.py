import pandas as pd
import os

from dynascale.baselines.lr import LinearRegression
# from dynascale.baselines.simple import Simple
from dynascale.systems.lds import LDSSystem
from dynascale.tasks import FixedComplexity, FixedError, FixedTrainSize


def main():
    n = 30
    task = FixedTrainSize(n=n,
                          L=[10, 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 100],
                          E=None,
                          T=[50],
                          max_control_cost_per_dim=0,
                          control_horizons=0,
                          test_examples=100,
                          reps=30,
                          test_timesteps=50,
                          system_cls=LDSSystem
                          )

    for in_dist in [True, False]:
        # file = f"../cache/LDS/simple_{in_dist=}.csv"
        file = f"../cache/LDS/LR/fts_{n=}_{in_dist=}.csv"
        if os.path.exists(file):
            data = pd.read_csv(file)
        else:
            data = task.evaluate(model_cls=LinearRegression, id=f"LR ({in_dist})", noisy=True, in_dist=in_dist)
            # data = task.evaluate(model_cls=Simple, id=f"Simple ({in_dist})", fit_kwargs={'epochs': 300}, noisy=True, in_dist=in_dist)
            data.to_csv(file)
        task.plot(data)


if __name__ == '__main__':
    main()
