...
class ARWHKSystem(AbstractSystem):
    def __init__(self, latent_dim, embed_dim,
        ...
        # Model selection
        self.model = op.ARWHKModel(g)

        # Model Configuration
        self.config = mc.Configuration()
        self.config.add_model_parameter("epsilon", 0.2)
        self.config.add_model_parameter("perc_stubborness", 0.1)
        self.config.add_model_parameter("option_for_stubbornness", 0)
        self.config.add_model_parameter("method_variant", 3)
        ...
        for e in edges:
            self.config.add_edge_configuration("weight", e, weight)
